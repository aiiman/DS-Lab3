...
# Install 'mice' package and test md.pattern() function
library(mice)
...
h3 <- ggplot(mice_imputed, aes(x = imputed_cart)) +
  geom_histogram(fill = "#1543ad", color = "#000000", position =
                   "identity") +
  ggtitle("Classification & regression trees") +
  theme_classic()
...
# Observation: Classification & regression trees resemble original distribution the most
